<x-layout.auth>
    <form class="bg-white rounded-md p-6 text-gray-900 space-y-4" action="{{ route('register.store') }}" method="POST">
        @csrf
        ...
        <div class="flex flex-col space-y-2">
            <label class="block text-gray-700 text-sm font-bold mb-2" for="password_confirmation">{{ __('Password Confirmation') }}</label>
            <input
            ...
            />
            @error('password_confirmation') <small class="text-red-500 text-xs italic">{{ $message }}</small> @enderror
        </div>
        <div>
            <button
